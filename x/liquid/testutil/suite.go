...
func (suite *Suite) CreateRedelegation(delegator sdk.AccAddress, fromValidator, toValidator sdk.ValAddress, amount sdk.Int) {
	stakingDenom := suite.StakingKeeper.BondDenom(suite.Ctx)
...
func (suite *Suite) EventsDoNotContainType(events sdk.Events, eventType string) {
	for _, event := range events {
