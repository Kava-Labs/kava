...
var MAX_AMOUNT = sdkmath.NewInt(1_000_000_000_000).SubRaw(1)
...
func (fb FractionalBalance) Validate() error {
	...
	if fb.Amount.IsNegative() {
		return fmt.Errorf("negative amount: %v", fb.Amount)
	}

	if fb.Amount.GT(MAX_AMOUNT) {
		return fmt.Errorf("amount exceeds max of %v: %v", MAX_AMOUNT, fb.Amount)
	}
