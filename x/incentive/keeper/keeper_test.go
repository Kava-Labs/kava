...
func (suite *KeeperTestSuite) TestIncrementSwapRewardAccrualTimes() {
	testCases := []struct {
		name            string
		poolName        string
		existingIndexes types.RewardIndexes
		increment       types.RewardIndexes
	}{
		{
			name:     "existing indexes are incremented",
			poolName: "btc/usdx",
			existingIndexes: types.RewardIndexes{
				{
					CollateralType: "hard",
			...
			increment: types.RewardIndexes{
				{
					CollateralType: "hard",
					RewardFactor:   d("0.001"),
				},
			},
		},
		{
			name:     "existing indexes are unchanged when increment is empty",
			poolName: "btc/usdx",
			existingIndexes: types.RewardIndexes{
				{
					CollateralType: "hard",
		...
		{
			name:            "non existant indexes are created and incremented",
			poolName:        "btc/usdx",
			existingIndexes: nil,
			increment: types.RewardIndexes{
				{
		...
		{
			name:            "added empty indexes to non existant indexes does nothing",
			poolName:        "btc/usdx",
			existingIndexes: nil,
			increment:       nil,
		},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if len(tc.existingIndexes) > 0 {
				suite.keeper.SetSwapRewardIndexes(suite.ctx, tc.poolName, tc.existingIndexes)
			}

			suite.keeper.IncrementSwapRewardIndexes(suite.ctx, tc.poolName, tc.increment)

			storedIndexes, found := suite.keeper.GetSwapRewardIndexes(suite.ctx, tc.poolName)
			if len(tc.existingIndexes) > 0 || len(tc.increment) > 0 {
				suite.True(found)
			}
			suite.Equal(tc.existingIndexes.Add(tc.increment), storedIndexes)
		})
	}
}
