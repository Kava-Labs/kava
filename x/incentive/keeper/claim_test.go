...
func (suite *ClaimTests) TestCannotClaimWhenMultiplierNotRecognised() {
	subspace := &fakeParamSubspace{
		params: types.Params{
			ClaimMultipliersTODO: types.MultipliersPerDenom{
				{
					Denom: "hard",
	...
	err := suite.keeper.ClaimDelegatorReward(suite.ctx, claim.Owner, claim.Owner, "hard", types.Large)
...
func (suite *ClaimTests) TestCannotClaimAfterEndTime() {
	...
	err := suite.keeper.ClaimDelegatorReward(suite.ctx, claim.Owner, claim.Owner, "hard", types.Small)
