...
func (suite *AccumulateBorrowRewardsTests) storedTimeEquals(poolID string, expected time.Time) {
	storedTime, found := suite.keeper.GetPreviousHardBorrowRewardAccrualTime(suite.ctx, poolID)
	suite.True(found)
