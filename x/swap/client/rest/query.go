...
func queryDepositsHandlerFn(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(pool) == 0 {
			rest.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("must specify param 'pool'"))
			return
		}
		...
		if len(owner) == 0 {
			rest.WriteErrorResponse(w, http.StatusBadRequest, fmt.Sprintf("must specify param 'owner'"))
			return
		}
