...
func atomicSwaps(count int) []types.AtomicSwap {
...
func atomicSwap(index int) types.AtomicSwap {
	...
	swap := types.NewAtomicSwap(cs(c("bnb", 50000)), randomNumberHash, expireOffset, timestamp, kavaAddrs[0],
		kavaAddrs[1], binanceAddrs[0], binanceAddrs[1], 1, types.SwapStatus_SWAP_STATUS_OPEN, true, types.SwapDirection_SWAP_DIRECTION_INCOMING)
