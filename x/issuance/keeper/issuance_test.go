...
func (suite *KeeperTestSuite) TestIssueTokens() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err := suite.keeper.IssueTokensInternal(suite.ctx, tc.args.tokens, sender, receiver)
...
func (suite *KeeperTestSuite) TestRedeemTokens() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err = suite.keeper.RedeemTokensInternal(suite.ctx, tc.args.redeemTokens, sender)

			if tc.errArgs.expectPass {
				suite.Require().NoError(err)
				initialSupply := sdk.NewCoins(tc.args.redeemTokens)
				moduleAccount := suite.getModuleAccount(types.ModuleAccountName)
				suite.Require().Equal(initialSupply.Sub(sdk.NewCoins(tc.args.redeemTokens)), sdk.NewCoins(suite.getBalance(moduleAccount.GetAddress(), tc.args.redeemTokens.Denom)))
			} else {
...
func (suite *KeeperTestSuite) TestBlockAddress() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err := suite.keeper.BlockAddressInternal(suite.ctx, tc.args.denom, sender, blockedAddr)
			if tc.errArgs.expectPass {
				suite.Require().NoError(err, tc.name)
...
func (suite *KeeperTestSuite) TestUnblockAddress() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			sender, _ := sdk.AccAddressFromBech32(tc.args.sender)
			blockedAddr, _ := sdk.AccAddressFromBech32(tc.args.blockedAddr)
			err := suite.keeper.UnblockAddressInternal(suite.ctx, tc.args.denom, sender, blockedAddr)
			if tc.errArgs.expectPass {
				suite.Require().NoError(err, tc.name)
				asset, found := suite.keeper.GetAsset(suite.ctx, tc.args.denom)
				blocked := false
...
func (suite *KeeperTestSuite) TestChangePauseStatus() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			sender, _ := sdk.AccAddressFromBech32(tc.args.sender)
			err := suite.keeper.SetPauseStatusInternal(suite.ctx, sender, tc.args.denom, tc.args.endStatus)
			if tc.errArgs.expectPass {
				suite.Require().NoError(err, tc.name)
				asset, found := suite.keeper.GetAsset(suite.ctx, tc.args.denom)
				suite.Require().True(found)
