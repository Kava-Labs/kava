...
// 	tApp := app.NewTestApp()
// 	authGS := app.NewAuthGenState([]sdk.AccAddress{addr1}, []sdk.Coins{{sdk.NewInt64Coin("foocoin", 67)}})
// 	tApp.InitializeFromGenesisStates(
// 		authGS,
// 	)

// 	ctxCheck := tApp.BaseApp.NewContext(true, abci.Header{})

// 	res1 := tApp.GetAccountKeeper().GetAccount(ctxCheck, addr1)
...
// 	header := abci.Header{Height: tApp.LastBlockHeight() + 1}
// 	simapp.SignCheckDeliver(t, tApp.Codec(), tApp.BaseApp, header, []sdk.Msg{sendMsg}, []uint64{origAccNum}, []uint64{origSeq}, false, false, priv1)

// 	res2 := tApp.GetAccountKeeper().GetAccount(tApp.NewContext(true, abci.Header{}), addr1)
...
// 			addrs := []sdk.AccAddress{test.msg.FromAddress}
// 			coins := []sdk.Coins{test.fromBalance}
...
// 			res1 := tApp.GetAccountKeeper().GetAccount(ctxCheck, test.msg.FromAddress)
// 			require.NotNil(t, res1)
...
// 			header := abci.Header{Height: tApp.LastBlockHeight() + 1}
// 			simapp.SignCheckDeliver(t, tApp.Codec(), tApp.BaseApp, header, []sdk.Msg{test.msg}, []uint64{origAccNum}, []uint64{origSeq}, test.expSimPass, test.expPass, priv1)

// 			fromBalance := tApp.GetAccountKeeper().GetAccount(ctxCheck, test.msg.FromAddress)
// 			toBalance := tApp.GetAccountKeeper().GetAccount(ctxCheck, test.msg.ToAddress)

// 			require.Equal(t, test.expFromBalance, fromBalance.GetCoins())
// 			require.Equal(t, test.expToBalance, toBalance.GetCoins())
