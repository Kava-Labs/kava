...
import (
	...
	keep "github.com/kava-labs/kava/x/bank/internal/keeper"
	"github.com/kava-labs/kava/x/bank/internal/types"
)

func TestKeeper(t *testing.T) {
	app := app.NewTestApp()
	ctx := app.NewContext(true, abci.Header{Height: 1, Time: tmtime.Now()})

	addr := sdk.AccAddress([]byte("addr1"))
	addr2 := sdk.AccAddress([]byte("addr2"))
	addr3 := sdk.AccAddress([]byte("addr3"))
	acc := app.GetAccountKeeper().NewAccountWithAddress(ctx, addr)

	// Test GetCoins/SetCoins
	app.GetAccountKeeper().SetAccount(ctx, acc)
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins()))

	app.GetBankKeeper().SetCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 10)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("foocoin", 10))))

	// Test HasCoins
	require.True(t, app.GetBankKeeper().HasCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 10))))
	require.True(t, app.GetBankKeeper().HasCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 5))))
	require.False(t, app.GetBankKeeper().HasCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 15))))
	require.False(t, app.GetBankKeeper().HasCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 5))))

	// Test AddCoins
	app.GetBankKeeper().AddCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 15)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("foocoin", 25))))

	app.GetBankKeeper().AddCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 15)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 15), sdk.NewInt64Coin("foocoin", 25))))

	// Test SubtractCoins
	app.GetBankKeeper().SubtractCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 10)))
	app.GetBankKeeper().SubtractCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 5)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 10), sdk.NewInt64Coin("foocoin", 15))))

	app.GetBankKeeper().SubtractCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 11)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 10), sdk.NewInt64Coin("foocoin", 15))))

	app.GetBankKeeper().SubtractCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 10)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("foocoin", 15))))
	require.False(t, app.GetBankKeeper().HasCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 1))))

	// Test SendCoins
	app.GetBankKeeper().SendCoins(ctx, addr, addr2, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 5)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("foocoin", 10))))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr2).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("foocoin", 5))))

	app.GetBankKeeper().SendCoins(ctx, addr, addr2, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 50)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("foocoin", 10))))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr2).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("foocoin", 5))))

	app.GetBankKeeper().AddCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 30)))
	app.GetBankKeeper().SendCoins(ctx, addr, addr2, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 10), sdk.NewInt64Coin("foocoin", 5)))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 20), sdk.NewInt64Coin("foocoin", 5))))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr2).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 10), sdk.NewInt64Coin("foocoin", 10))))
	...
	app.GetBankKeeper().InputOutputCoins(ctx, []types.Input{input1}, []types.Output{output1})
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 20), sdk.NewInt64Coin("foocoin", 7))))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr2).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 10), sdk.NewInt64Coin("foocoin", 8))))
	...
	app.GetBankKeeper().InputOutputCoins(ctx, inputs, outputs)
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 21), sdk.NewInt64Coin("foocoin", 4))))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr2).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 7), sdk.NewInt64Coin("foocoin", 6))))
	require.True(t, app.GetBankKeeper().GetCoins(ctx, addr3).IsEqual(sdk.NewCoins(sdk.NewInt64Coin("barcoin", 2), sdk.NewInt64Coin("foocoin", 5))))

	// Test retrieving black listed accounts
	for acc := range simapp.GetMaccPerms() {
		addr := supply.NewModuleAddress(acc)
		require.Equal(t, app.BlacklistedAccAddrs()[addr.String()], app.GetBankKeeper().BlacklistedAddr(addr))
	}
}
...
func TestSendKeeper(t *testing.T) {
	...
	paramSpace := app.GetParamsKeeper().Subspace("newspace")
	sendKeeper := keep.NewBaseSendKeeper(app.GetAccountKeeper(), paramSpace, blacklistedAddrs)
	app.GetBankKeeper().SetSendEnabled(ctx, true)
	...
	app.GetBankKeeper().SetCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 15)))
...
func TestInputOutputNewAccount(t *testing.T) {
	...
	acc1 := app.GetAccountKeeper().NewAccountWithAddress(ctx, addr1)

	app.GetAccountKeeper().SetAccount(ctx, acc1)
	require.NoError(t, app.GetBankKeeper().SetCoins(ctx, addr1, balances))

	acc1Balances := app.GetBankKeeper().GetCoins(ctx, addr1)
	require.Equal(t, balances, acc1Balances)

	addr2 := sdk.AccAddress([]byte("addr2"))

	require.Nil(t, app.GetAccountKeeper().GetAccount(ctx, addr2))
	require.Empty(t, app.GetBankKeeper().GetCoins(ctx, addr2))
	...
	require.NoError(t, app.GetBankKeeper().InputOutputCoins(ctx, inputs, outputs))

	expected := sdk.NewCoins(sdk.NewInt64Coin("foo", 30), sdk.NewInt64Coin("bar", 10))
	acc2Balances := app.GetBankKeeper().GetCoins(ctx, addr2)
	require.Equal(t, expected, acc2Balances)
	require.NotNil(t, app.GetAccountKeeper().GetAccount(ctx, addr2))
}
...
func TestMsgSendEvents(t *testing.T) {
	...
	err := app.GetBankKeeper().SendCoins(ctx, addr, addr2, newCoins)
	require.Error(t, err)
	...
	app.GetBankKeeper().SetCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 50)))
	newCoins = sdk.NewCoins(sdk.NewInt64Coin("foocoin", 50))
	err = app.GetBankKeeper().SendCoins(ctx, addr, addr2, newCoins)
...
func TestMsgMultiSendEvents(t *testing.T) {
	...
	acc := app.GetAccountKeeper().NewAccountWithAddress(ctx, addr)
	acc2 := app.GetAccountKeeper().NewAccountWithAddress(ctx, addr2)

	app.GetAccountKeeper().SetAccount(ctx, acc)
	app.GetAccountKeeper().SetAccount(ctx, acc2)
	newCoins := sdk.NewCoins(sdk.NewInt64Coin("foocoin", 50))
	...
	err := app.GetBankKeeper().InputOutputCoins(ctx, inputs, outputs)
	require.Error(t, err)
	events := ctx.EventManager().Events()
	...
	app.GetBankKeeper().SetCoins(ctx, addr, sdk.NewCoins(sdk.NewInt64Coin("foocoin", 50)))
	newCoins = sdk.NewCoins(sdk.NewInt64Coin("foocoin", 50))
	app.GetBankKeeper().SetCoins(ctx, addr2, sdk.NewCoins(sdk.NewInt64Coin("barcoin", 100)))
	newCoins2 = sdk.NewCoins(sdk.NewInt64Coin("barcoin", 100))
...
func TestViewKeeper(t *testing.T) {
	...
	//paramSpace := app.GetParamsKeeper().Subspace(types.DefaultParamspace)
	viewKeeper := keep.NewBaseViewKeeper(app.GetAccountKeeper())
...
func TestVestingAccountSend(t *testing.T) {
	...
	app.GetAccountKeeper().SetAccount(ctx, vacc)

	// require that no coins be sendable at the beginning of the vesting schedule
	err := app.GetBankKeeper().SendCoins(ctx, addr1, addr2, sendCoins)
	require.Error(t, err)
	...
	err = app.GetBankKeeper().SendCoins(ctx, addr1, addr2, sendCoins)
	vacc = app.GetAccountKeeper().GetAccount(ctx, addr1).(*vesting.ContinuousVestingAccount)
	require.NoError(t, err)
...
func TestPeriodicVestingAccountSend(t *testing.T) {
	...
	ctx = ctx.WithBlockTime(now.Add(12 * time.Hour))
	err = app.GetBankKeeper().SendCoins(ctx, addr1, addr2, sendCoins)
	vacc = app.GetAccountKeeper().GetAccount(ctx, addr1).(*vesting.PeriodicVestingAccount)
	require.NoError(t, err)
	require.Equal(t, origCoins, vacc.GetCoins())
}
...
func TestVestingAccountReceive(t *testing.T) {
	...
	bacc.SetCoins(origCoins)
	vacc := vesting.NewContinuousVestingAccount(&bacc, ctx.BlockHeader().Time.Unix(), endTime.Unix())
	acc := app.GetAccountKeeper().NewAccountWithAddress(ctx, addr2)
	app.GetAccountKeeper().SetAccount(ctx, vacc)
	app.GetAccountKeeper().SetAccount(ctx, acc)
	app.GetBankKeeper().SetCoins(ctx, addr2, origCoins)

	// send some coins to the vesting account
	app.GetBankKeeper().SendCoins(ctx, addr2, addr1, sendCoins)
...
func TestDelegateCoins(t *testing.T) {
	...
	err := app.GetBankKeeper().DelegateCoins(ctx, addr2, addrModule, delCoins)
	...
	err = app.GetBankKeeper().DelegateCoins(ctx, addr1, addrModule, delCoins)
...
func TestUndelegateCoins(t *testing.T) {
	...
	err = app.GetBankKeeper().UndelegateCoins(ctx, addrModule, addr2, delCoins)
	require.NoError(t, err)
	...
	err = app.GetBankKeeper().UndelegateCoins(ctx, addrModule, addr1, delCoins)
	require.NoError(t, err)
