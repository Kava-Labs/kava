...
func (k Keeper) CdpDenomIndexIterator(ctx sdk.Context, denom string) sdk.Iterator {
	...
	return sdk.KVStorePrefixIterator(store, []bytes{denom})
}
...
func (k Keeper) CdpLiquidationRatioIndexIterator(ctx sdk.Context, denom string, targetRatio sdk.Dec) {
	store := prefix.NewStore(ctx.KVStore(k.key), types.CollateralRatioIndexPrefix)
	return store.Iterator(SortableDecBytes(sdk.NewDec(0)), types.LiquidationRatioKey(targetRatio))
}
...
func (k Keeper) IterateCdpsByLiquidationRatio(ctx sdk.Context, denom string, targetRatio sdk.Dec, cb func(cdp types.CDP) (stop bool)) {
	...
	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		d, _, id := types.SplitCollateralRatioKey(iterator.Key())
