...
func (suite *hookTestSuite) TestHooks_DepositAndWithdraw() {
	...
	earnHooks.On(
		...
		acc.GetAddress(),
		sdkmath.LegacyNewDecFromInt(acc1deposit1Amount.Amount),
	).Once()
	...
	earnHooks.On(
		...
		acc2.GetAddress(),
		sdkmath.LegacyNewDecFromInt(acc2deposit1Amount.Amount),
	).Once()
	err = suite.Keeper.Deposit(
		suite.Ctx,
		acc2.GetAddress(),
	...
	earnHooks.On(
		...
		acc2.GetAddress(),
		sdkmath.LegacyNewDecFromInt(acc2deposit2Amount.Amount),
	).Once()
	err = suite.Keeper.Deposit(
		suite.Ctx,
		acc2.GetAddress(),
...
func (suite *hookTestSuite) TestHooks_HookOrdering() {
	...
	earnHooks.On("AfterVaultDepositCreated", suite.Ctx, depositAmount.Denom, acc.GetAddress(), sdkmath.LegacyNewDecFromInt(depositAmount.Amount)).
		Run(func(args mock.Arguments) {
			...
			suite.Require().Equal(sdkmath.LegacyNewDecFromInt(depositAmount.Amount), shares.AmountOf(depositAmount.Denom))
		})
	err := suite.Keeper.Deposit(suite.Ctx, acc.GetAddress(), depositAmount, types.STRATEGY_TYPE_HARD)
	suite.Require().NoError(err)

	earnHooks.On("BeforeVaultDepositModified", suite.Ctx, depositAmount.Denom, acc.GetAddress(), sdkmath.LegacyNewDecFromInt(depositAmount.Amount)).
		Run(func(args mock.Arguments) {
			shares, found := suite.Keeper.GetVaultAccountShares(suite.Ctx, acc.GetAddress())
			suite.Require().True(found, "expected after hook to be called after shares are updated")
			suite.Require().Equal(sdkmath.LegacyNewDecFromInt(depositAmount.Amount), shares.AmountOf(depositAmount.Denom))
	...
	earnHooks.On("BeforeVaultDepositModified", suite.Ctx, depositAmount.Denom, acc.GetAddress(), existingShares.AmountOf(depositAmount.Denom)).
		Run(func(args mock.Arguments) {
			shares, found := suite.Keeper.GetVaultAccountShares(suite.Ctx, acc.GetAddress())
			suite.Require().True(found, "expected after hook to be called after shares are updated")
			suite.Require().Equal(sdkmath.LegacyNewDecFromInt(depositAmount.Amount.MulRaw(2)), shares.AmountOf(depositAmount.Denom))
		})
