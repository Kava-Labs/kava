...
func (k *Keeper) Withdraw(
	...
	sendCoins := func(ctx sdk.Context, amount sdk.Coin) error {
	...
	_, err := k.withdraw(ctx, from, wantAmount, withdrawStrategy, sendCoins)
	return err
}

// withdraw removes the amount of supplied tokens from a vault and transfers it
// back to the account. The sendCoins function is resposible for sending coins
// from the earn module account to the user's account.
func (k *Keeper) withdraw(
	ctx sdk.Context,
	from sdk.AccAddress,
	wantAmount sdk.Coin,
	...
	if err := sendCoins(ctx, withdrawAmount); err != nil {
		return sdk.Coin{}, err
	}
...
// WithdrawToModuleAccount removes the amount of supplied tokens from a vault and transfers it
...
func (k *Keeper) WithdrawFromModuleAccount(ctx sdk.Context, from string, wantAmount sdk.Coin, withdrawStrategy types.StrategyType) (sdk.Coin, error) {
	...
	fromAddr := k.accountKeeper.GetModuleAddress(from)
	return k.withdraw(ctx, fromAddr, wantAmount, withdrawStrategy, sendCoins)
}
