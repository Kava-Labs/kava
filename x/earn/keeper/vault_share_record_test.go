...
func (suite *vaultTestSuite) TestGetVaultRecord() {
	record := types.NewVaultRecord("usdx", sdkmath.LegacyZeroDec())
...
func (suite *vaultTestSuite) TestUpdateVaultRecord() {
	...
	record.TotalShares = types.NewVaultShare("usdx", sdkmath.LegacyNewDec(100))
	...
	// Remove supply
	record.TotalShares = types.NewVaultShare("usdx", sdkmath.LegacyNewDec(0))
	suite.Keeper.UpdateVaultRecord(suite.Ctx, record)
...
func (suite *vaultTestSuite) TestGetVaultShareRecord() {
	...
	record.Shares = types.NewVaultShares(
		types.NewVaultShare(vaultDenom, sdkmath.LegacyNewDec(100)),
	)
