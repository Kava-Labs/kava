...
# abi generates EVM ABI and bytecode for Mul3Caller contract
abi:
	solc --via-ir --evm-version paris --abi --bin ./contracts/contracts/mul3_caller/Mul3Caller.sol -o ./contracts/contracts/mul3_caller --overwrite
	cp ./contracts/contracts/mul3_caller/IMul3.abi ./precompile/contracts/mul3/IMul3.abi

# abigen generates golang library for deploying and interacting with Mul3Caller contract
# abigen depends on ABI and bytecode of Mul3Caller contract (which can be generated by make abi command)
abigen:
	abigen \
		--abi=./contracts/contracts/mul3_caller/Mul3Caller.abi \
		--bin=./contracts/contracts/mul3_caller/Mul3Caller.bin \
		--pkg=mul3_caller \
		--out=./contracts/contracts/mul3_caller/mul3_caller.go
