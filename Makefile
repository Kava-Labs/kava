...
# abi generates EVM ABI and bytecode for NoopCaller contract
abi:
	solc --via-ir --evm-version paris --abi --bin ./contracts/contracts/noop_caller/NoopCaller.sol -o ./contracts/contracts/noop_caller --overwrite

# abigen generates golang library for deploying and interacting with NoopCaller contract
# abigen depends on ABI and bytecode of NoopCaller contract (which can be generated by make abi command)
abigen:
	abigen \
		--abi=./contracts/contracts/noop_caller/NoopCaller.abi \
		--bin=./contracts/contracts/noop_caller/NoopCaller.bin \
		--pkg=noop_caller \
		--out=./contracts/contracts/noop_caller/noop_caller.go
