...
func TestMigrateIncentive(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Len(t, genstate.DelegatorClaims, 0)
	assert.Len(t, genstate.USDXMintingClaims, 0)
	assert.Len(t, genstate.HardLiquidityProviderClaims, 0)
	assert.Len(t, genstate.DelegatorClaims, 0)
	assert.Len(t, genstate.SwapClaims, 0)
