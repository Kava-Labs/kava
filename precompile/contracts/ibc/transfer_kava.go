...
func transferKava(
	...
	input, err := UnpackTransferKavaInput(packedInput)
	if err != nil {
	...
	_, err = stateDB.IBCTransferKeeper().Transfer(stateDB.Context(), &ibctransfertypes.MsgTransfer{
		...
		Token: sdk.Coin{
			Denom:  "ukava",
			Amount: sdkmath.NewIntFromBigInt(value),
		},
...
// UnpackTransferKavaInput attempts to unpack [input] into the *TransferKavaInput type argument
// assumes that [input] does not include selector (omits first 4 func signature bytes)
func UnpackTransferKavaInput(packedInput []byte) (*TransferKavaInput, error) {
	res, err := IBCABI.UnpackInput("transferKava", packedInput)
	if err != nil {
		return nil, err
	}

	var input TransferKavaInput
	input.sourcePort = *abi.ConvertType(res[0], new(string)).(*string)
	input.sourceChannel = *abi.ConvertType(res[1], new(string)).(*string)

	input.receiver = *abi.ConvertType(res[2], new(string)).(*string)

	input.revisionNumber = *abi.ConvertType(res[3], new(uint64)).(*uint64)
	input.revisionHeight = *abi.ConvertType(res[4], new(uint64)).(*uint64)
	input.timeoutTimestamp = *abi.ConvertType(res[5], new(uint64)).(*uint64)
	input.memo = *abi.ConvertType(res[6], new(string)).(*string)
	return &input, nil
}
