...
var (
	...
	mAccPerms = map[string][]string{
		...
		liquidstakingtypes.ModuleAccountName: {authtypes.Burner}, // TODO minter/burner?
	}
)
...
func (app *App) loadBlockedMaccAddrs() map[string]bool {
	...
	liquidStakingMaccAddr := app.accountKeeper.GetModuleAddress(liquidstakingtypes.ModuleAccountName)
	for addr := range modAccAddrs {
		...
		// TODO unblock the liquid staking module account so it can claim rewards
		if addr == liquidStakingMaccAddr.String() {
			modAccAddrs[addr] = false
		}
	}
	return modAccAddrs
