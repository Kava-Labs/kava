...
func (spkd SetPubKeyDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for i, pk := range pubkeys {
		...
		// pubkey is ethsecp256k1 and account is base account - convert to eth account
		if isEthPubkey && isBaseAcc {
			...
			spkd.ak.SetAccount(ctx, ethAcc)
		} else
		// for other account types (vesting accounts), even if the pubkey is eth pubkey,
		// we can't convert to eth account, because we would lose vesting data
		{
